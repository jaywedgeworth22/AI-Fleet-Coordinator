...
| `~/apps/update-botfleet.sh` | On-demand.  Moves `~/apps/botfleet-server` to `origin/main` (refuses a checkout with local edits), packages and installs `/Applications/BotFleet.app` from it, signs with the stable local Developer ID / Development certificate so macOS Keychain secret storage survives rebuilds, restarts `com.jay.botfleet-server`, waits for `/api/health`, then relaunches the app.  Rewritten 2026-09-02 (Claude); previous version at `update-botfleet.sh.bak-20260902`. |  Launchers: terminal alias `ubf` (`~/.zshrc`) runs this script directly; the double-click wrapper `~/apps/Update BotFleet.command` was moved off the Desktop on Wed, Sep 3, 2026 and calls the same script.  Its clean-checkout guard restores the two Swift helper binaries (`BotFleet Recorder.app` / `BotFleet Speech.app`) and `electron/vendor/electron-updater.cjs` before judging, since those are its own build output — updated Sun, Sep 13, 2026.  Sentry DSN (Mon, Sep 7, 2026, Claude): after `pnpm install` it exports `SENTRY_DSN` / `VITE_SENTRY_DSN` / `BOTFLEET_SENTRY_DSN` from `~/.secrets/botfleet-sentry.env` (chmod 600, values never printed; only the length is logged) so the renderer inlines the DSN and the bundled server reads it at runtime; a missing file prints a loud warning and the build ships without Sentry.  Previous version at `update-botfleet.sh.bak-20260908`.  `ubf --force` (same day, Claude) skips the "already at origin/main" early exit: the checkout can be moved to `origin/main` by something other than this script (observed twice on Mon, Sep 7, 2026 with the harness and the installed app still on older code), and the HEAD comparison alone cannot see that; the early-exit message now says so.  Signed packaging retries once with the same identity and `-c.mac.timestamp=none` when codesign fails on Apple's timestamp server ("A timestamp was expected but was not found", seen Mon, Sep 7, 2026); a local `--dir` build is never notarized and does not need the timestamp.
