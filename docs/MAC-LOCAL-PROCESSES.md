...
Last inventory: Sat, Sep 12, 2026 6:50pm CT (Claude).  Installed the repo's transactional Mac updater as `~/apps/update-botfleet.sh` — a copy of `scripts/update-botfleet.sh` from BotFleet `3b30294e` that execs the tracked `update-botfleet-mac.mjs` in `~/apps/botfleet-server`, replacing the old `rm -rf` / `cp -a` script (kept at `update-botfleet.sh.bak-20260912-safe-updater`).  The `ubf` alias and the double-click `Update BotFleet.command` are unchanged, `ubf --force` is accepted and ignored, and no update was run: `/Applications/BotFleet.app` is still 1.0.30 and the harness was not restarted.  Prior: Sat, Sep 12, 2026 1:20pm CT (Claude).  `com.jay.botfleet-server` wedged (routines.json had grown to 35.7 MB of per-run prompt snapshots; every synchronous save stalled the event loop under 14 GB of swap, so the desktop app saw connection resets and showed its error page), then crash-looped on restart because `com.jay.disk-janitor` had reaped `~/apps/botfleet-server/node_modules` as an idle clean worktree.  Fixed: `~/apps/botfleet-server` added to the janitor `KEEP_RE` (live + tracked `scripts/disk-janitor.sh`), routines.json compacted to 4.8 MB (backup `~/.botfleet/routines.json.bak-prompt-bloat-20260912-203847`), dependencies reinstalled, harness restarted; six orphaned 4.4 GB idle `node` processes (ppid 1) killed to end the swap thrash.  BotFleet PR `claude/boot-error-page` keeps prompt snapshots to the newest 100 settled runs and names the unresponsive owner on the error page.  Prior: Wed, Sep 9, 2026 7:32am CET (MINIMAX).  Retired Mac-side `residential-proxy`, `senate-relay`, `senate-tunnel` (pm2 jobs already down; source files at `~/Code/Congress.Trade/scout/{residential-proxy.mjs,run-residential-proxy.sh,senate-relay.ts,run-senate-relay.sh,run-senate-tunnel.sh,congress-scout.mjs,liveness.ts,run-scout.sh,README.md,scout-state*}` and the pinned runtime `~/apps/senate-relay-runtime/` deleted in the same change).  CT server now does Senate relay, latency probes, and scout work; the physical-device residential proxy is the only res-IP bounce path.  Coolify server is the manager of record for everything that used to live on this Mac.  Prior: Tue, Sep 8, 2026 10:05pm CT (Claude).  Added on-demand `~/apps/fleet-rag/recall-tunnel` (SSH tunnel to the Hetzner box for Qdrant/TEI/rerank when Tailscale is down; AFC #202) and reinstalled `~/apps/fleet-rag/` plus the two fleet-recall Claude Code hooks from that PR (SessionStart hook never announces or caches 0 points; `recall doctor` matches the Oracle routine names case-insensitively and fails fast instead of retrying for minutes).  Tailscale is still logged out on this Mac (x509 on the current network), so the nightly Oracle ingest cannot reach Qdrant until `tailscale login` succeeds or the routine opens the tunnel first.  Prior: Mon, Sep 7, 2026 9:45pm CT (Claude).  `~/apps/update-botfleet.sh` now exports `SENTRY_DSN` / `VITE_SENTRY_DSN` / `BOTFLEET_SENTRY_DSN` from the chmod 600 handoff file `~/.secrets/botfleet-sentry.env` before packaging (values never printed; loud warning when the file is missing) so the locally built desktop app and its bundled server report to Sentry project `jays-services/botfleet`.  Previous version at `update-botfleet.sh.bak-20260908`.  The LaunchAgent plist is deliberately unchanged: the harness takes its DSN from `~/.botfleet/config.json` → `observability.sentryDsn` (Settings → Observability) once BotFleet PR `claude/sentry-usage-telemetry` lands, and env would shadow that card.  Prior: Mon, Sep 8, 2026 8:20pm CT (BF-FIXER).  `~/apps/botfleet-imessage-relay.py` now POSTs inbound with `source: "imessage"` and relays only bot replies that start with `[to iMessage]` (tag stripped).  LaunchAgent `com.jay.botfleet-imessage-relay` stays Retired/Forbidden under jay — do not load it.  Prior: Wed, Sep 2, 2026 9:42pm CT (Antigravity).  Booted out `com.jay.botfleet-imessage-relay` after an infinite echo loop between Jay's account and the `agents` account (sending outgoing iMessages to HOUSEKEEPER and triggering macOS send sounds every 15s).  Prior 6:40pm CT (Claude).  Added fleet-recall Claude Code hooks, the `fleet-recall` stdio MCP row, and the Oracle RAG routines.  Prior 2:45pm CT (Claude).  `com.jay.botfleet-server` now runs from `~/apps/botfleet-server` (detached `origin/main` worktree) instead of the stale `botfleet-grok` lane, and `~/apps/update-botfleet.sh` builds from that checkout and restarts the harness.  Prior inventory, Sep 2 (Grok): Idle unload **36h → 12h** (`GROK_IDLE_UNLOAD_HOURS`; `/resume` reloads tools).  Prior 11:30pm: added hourly `com.jay.grok-idle-unload` (session/close live Grok chats; transcripts kept).  Hetzner swap target 16GiB + swappiness 20.  Prior 10:40pm: `com.jay.botfleet-imessage-relay` is **Up** (12 bot chats mapped; log no longer doubled).  Earlier 8:40pm: added on-demand `~/apps/fleet-rag/recall` + `scripts/install-fleet-rag.sh`.  Corrected com.jay.botfleet-imessage-relay: LaunchAgent missing/unloaded, not Up.  Added `com.jay.mac-resource-watch` (every 5 min, disk/RAM/CPU → cleanup + Housekeeper webhook).  Janitor warn 80G / pressure 65G.  Disk janitor now walks all fleet Code repos (was ST/CT/CTS only), protects unsuffixed standing lanes + runtimes, reaps clean kimi-named / nested / `/tmp` trees, and uses 2-day idle when free < 50G.  Live `~/.claude-disk-janitor/janitor.sh`; tracked `scripts/disk-janitor.sh`.  `mac-collab-writeback` is in the ecosystem (15 pm2 apps) and watch expected list.  Hardened after the first-run loop (updated_at from sync POSTs → gh close/reopen of ~1700 issues/cycle; lossy md re-render).  Writeback now bootstraps an applied-status map, edits live logs surgically, uses REST + current-state check for Issues, and does not git-commit `~/Code/<repo>`.  Tracked copies: `ai-fleet-coordinator/scripts/mac-process-watch.sh` + `scripts/pm2-ecosystem.config.cjs` + `scripts/mac-collab/write_back.py` + `scripts/grok-leader.sh` + `scripts/mac-status.sh` + `scripts/disk-janitor.sh`.  Command of record: `bash ~/apps/mac-status.sh`.
...
| **pm2 `dsh-web`** | Always-on | Harness browser UI on `127.0.0.1:3080` (display name "Harness"; upstream is `@deepseek-ai/dsh`).  Tailscale Serve `https://macbook.boa-roygbiv.ts.net:3080` (tailnet only, no Funnel).  Start: `~/apps/dsh-runtime/start-web.sh`.  Serve: `~/apps/dsh-runtime/serve-tailscale.sh`.  **Never `npx @deepseek-ai/dsh`.**  Added 2026-09-01. | **Up** |
...
| `~/apps/dsh-runtime/` | Pinned Harness (display name; upstream is `@deepseek-ai/dsh`).  **Never `npx @deepseek-ai/dsh`.**  Web UI is always-on pm2 `dsh-web` on **:3080** (~12 MB RSS / 0% CPU idle — cheap enough to leave up).  Dock app: `~/Applications/Harness.app` (bundle id `com.jays.dsh-harness-web`) is a WKWebView shell (running-dot, second click focuses the same window).  Rebuild: `~/apps/dsh-runtime/install-dock-app.sh`.  `dsh-acp.sh` is Shellular id `deepseek`. |
