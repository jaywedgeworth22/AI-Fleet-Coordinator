...
| **pm2 `dsh-web`** | Always-on | Harness browser UI on `127.0.0.1:3080` (display name "Harness"; upstream is `@deepseek-ai/dsh`).  Tailscale Serve `https://macbook.boa-roygbiv.ts.net:3080` (tailnet only, no Funnel).  Start: `~/apps/dsh-runtime/start-web.sh`.  Serve: `~/apps/dsh-runtime/serve-tailscale.sh`.  CLI wrapper `~/apps/dsh-runtime/dsh.sh` execs `node_modules/.bin/dsh` only — never this file, never `npx`.  Dock `pingHarness` / `ensure-web.sh` use an 8s probe and do not restart a listener on :3080.  Restore deps with `npm ci` in `~/apps/dsh-runtime`.  **Never `npx @deepseek-ai/dsh`.**  Added 2026-09-01.  Self-exec + 2s ping fix 2026-09-17. | **Up** |
...
| `~/.claude/hooks/secret-guard-pretooluse.py` | Always-on (Claude Code PreToolUse hook, matcher `Bash`, registered in `~/.claude/settings.json`).  Hard-denies secret-exposure command structure: dump of a secrets file (cat/head/tail/less/more/bat plus od/xxd/hexdump/hd/strings/base64/`cut -c`), Bearer args with no redaction pipe, any `ps`, `pgrep -l`/`-fl`/`-lf`, secrets-path plus `2>&1`, and byte-dump / prefix-cut / last-command `printf %s` of `$NAME` when NAME looks like KEY/TOKEN/SECRET/PASSWORD/DSN.  2026-09-17 gap: `od -c` on a loaded `SILICONFLOW_API_KEY` printed the live value; Rule E closes that.  Fails open on malformed input.  Tracked `scripts/hooks/secret-guard-pretooluse.py`.  Added with ps/file rules earlier; Rule E 2026-09-18 (Grok). |
...
| `~/apps/dsh-runtime/` | Pinned Harness (display name; upstream is `@deepseek-ai/dsh`).  **Never `npx @deepseek-ai/dsh`.**  CLI is `dsh.sh` → `node_modules/.bin/dsh` (tracked `scripts/dsh-runtime/dsh.sh`).  Web UI is always-on pm2 `dsh-web` on **:3080** (~12 MB RSS / 0% CPU idle — cheap enough to leave up).  Dock app: `~/Applications/Harness.app` (bundle id `com.jays.dsh-harness-web`) is a WKWebView shell (running-dot, second click focuses the same window).  Rebuild: `~/apps/dsh-runtime/install-dock-app.sh`.  `dsh-acp.sh` is Shellular id `deepseek`.  Empty `node_modules` → `npm ci` in this dir, then `pm2 restart dsh-web`. |
