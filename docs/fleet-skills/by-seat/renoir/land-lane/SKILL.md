---
name: land-lane
description: Land a Renoir feature branch to main across the fleet — seat worktree, docs, verification gate, PR, auto-merge, then closeout. Use when finishing a unit, opening a PR, merging, or when the owner says land/ship/commit/push. Never wait for the owner to ask. Covers apps with and without scripts/land.sh.
---
...
Seat: **RENOIR**.  Branch: `renoir/<slug>`.  Never `claude/`.  Never `monet/`.  Never land from `~/Code/<repo>` or from branch `main`.

## Preconditions

1. You are in your Renoir worktree (`~/apps/<prefix>-renoir` or `~/apps/<prefix>-renoir-<lane>`).  See `session-start`.
...
4. Substantial owner-facing work: living Apple Note via the `apple-notes` skill, title `[APP, Renoir] …`.
...
If the box is gating several lanes, post `[RENOIR] gating now` with `repo:` (not `->FLEET` unless every listening seat on every platform must spend time).  Coordinator/ops self-id is `AFC`.
