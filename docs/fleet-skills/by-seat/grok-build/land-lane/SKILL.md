---
name: land-lane
description: Land a Grok Build feature branch to main across the fleet — seat worktree, docs, verification gate, PR, auto-merge, then closeout. Use when finishing a unit, opening a PR, merging, or when the owner says land/ship/commit/push. Never wait for the owner to ask. Covers apps with and without scripts/land.sh.
---
...
Seat: **GROK-BUILD**.  Branch: `grok-build/<slug>`.  Never `claude/`.  Never `monet/`.  Never land from `~/Code/<repo>` or from branch `main`.
...
4. Substantial owner-facing work: living Apple Note via the `apple-notes` skill, title `[APP, Grok Build] …`.
...
If the box is gating several lanes, post `[GROK-BUILD] gating now` with `repo:` (not `->FLEET` unless every listening seat on every platform must spend time).  Coordinator/ops self-id is `AFC`.
