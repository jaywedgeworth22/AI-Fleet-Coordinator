---
name: land-lane
description: Land a Cursor feature branch to main across the fleet — seat worktree, docs, verification gate, PR, auto-merge, then closeout. Use when finishing a unit, opening a PR, merging, or when the owner says land/ship/commit/push. Never wait for the owner to ask. Covers apps with and without scripts/land.sh.
---
...
Seat: **CURSOR**.  Branch: `cursor/<slug>`.  Never `claude/`.  Never `monet/`.  Never land from `~/Code/<repo>` or from branch `main`.

## Preconditions

1. You are in your Cursor worktree (`~/apps/<prefix>-cursor` or `~/apps/<prefix>-cursor-<lane>`).  See `session-start`.
...
4. Substantial owner-facing work: living Apple Note via the `apple-notes` skill, title `[APP, Cursor] …`.
...
If the box is gating several lanes, post `[CURSOR] gating now` with `repo:` (not `->FLEET` unless every listening seat on every platform must spend time).  Coordinator/ops self-id is `AFC`.
